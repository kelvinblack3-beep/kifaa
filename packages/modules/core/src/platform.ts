 ...
 *
 * Environment: SANDBOX / NON-PRODUCTION / NON-CUSTODIAL product stance for live rails.
 * Internal balances are double-entry ledger projections for test money only.
 *
 * V0.1 money movement is **ledger-only sandbox simulation**:
 * - Does not execute SandboxProvider.fund/cashIn/cashOut (or any external rail).
 * - Does not simulate external provider settlement.
 * - SandboxProvider remains a future boundary/reference implementation only.
 * - All CoreTransaction records carry sandbox: true and mode is SANDBOX.
 */
...
export class KifaaPlatform {
  ...
  /** @internal exposed for integrity tests only */
  ...
  constructor(opts: KifaaPlatformOptions = {}) {
    ...
    this.ledger.registerAccount({
      ...
      type: "suspense",
      currency: "KES",
      label: "System suspense",
    });

    this.ledger.postJournal({
      id: newJournalId(),
      description: "Initialize sandbox float (SANDBOX ONLY — ledger projection)",
      lines: [
  ...
  addTestMoney(input: AddTestMoneyInput): CoreTransaction {
    ...
    const txn = this.beginTxn({
      ...
      receiverAccountId: user.ledgerAccountId,
      receiverKifaaId: user.kifaaId,
    ...
    return this.completeWithJournal(txn, () =>
      this.post({
        description: `SANDBOX Add test money ${txn.reference}`,
  ...
  transfer(input: TransferInput): CoreTransaction {
    ...
    const txn = this.beginTxn({
      type: "TRANSFER",
      amountMinor: input.amountMinor,
      senderAccountId: sender.ledgerAccountId,
      receiverAccountId: receiver.ledgerAccountId,
      senderKifaaId: sender.kifaaId,
      receiverKifaaId: receiver.kifaaId,
      idempotencyKey: input.idempotencyKey,
      requestHash: hashBody(body),
    ...
    return this.completeWithJournal(txn, () =>
      this.post({
        description: `Transfer ${txn.reference}`,
        transactionId: txn.id,
        lines: [
  ...
  payMerchant(input: MerchantPayInput): CoreTransaction {
    ...
    const txn = this.beginTxn({
      type: "MERCHANT_PAY",
      amountMinor: input.amountMinor,
      senderAccountId: payer.ledgerAccountId,
      receiverAccountId: merchant.ledgerAccountId,
      senderKifaaId: payer.kifaaId,
    ...
    return this.completeWithJournal(txn, () =>
      this.post({
        description: `Merchant pay ${txn.reference}`,
        transactionId: txn.id,
        lines: [
  ...
  agentCashIn(input: AgentCashInInput): CoreTransaction {
    ...
    if (agent.status !== "ACTIVE") {
      throw new ValidationError(`Agent is ${agent.status}`);
    }
    ...
    const txn = this.beginTxn({
      type: "AGENT_CASH_IN",
      amountMinor: input.amountMinor,
      receiverAccountId: customer.ledgerAccountId,
      receiverKifaaId: customer.kifaaId,
  ...
  agentCashOut(input: AgentCashOutInput): CoreTransaction {
    ...
    const txn = this.beginTxn({
      type: "AGENT_CASH_OUT",
      amountMinor: input.amountMinor,
      senderAccountId: customer.ledgerAccountId,
      senderKifaaId: customer.kifaaId,
      agentId: agent.agentId,
      idempotencyKey: input.idempotencyKey,
      requestHash: hashBody(body),
      metadata: { sandbox: true, provider: this.sandbox.code },
    ...
    return this.completeWithJournal(txn, () =>
      this.post({
        description: `Agent cash-out ${txn.reference}`,
        transactionId: txn.id,
        lines: [
  ...
  reverse(transactionId: string, reason: string): CoreTransaction {
    ...
    const rev: CoreTransaction = {
      ...
      requestHash: hashBody({ reverses: original.id }),
      metadata: { reverses: original.id, reason, sandbox: true },
      sandbox: true,
  ...
  receipt(transactionId: string): {
    ...
    return {
      ...
      currency: "KES",
      sender: t.senderKifaaId,
      receiver: t.receiverKifaaId,
  ...
  private idempotentLookup(key: string, body: unknown): CoreTransaction | null {
    ...
    if (existing.status === "FAILED" && !existing.journalId) {
      this.audit("txn.idempotency_retry_allowed", undefined, {
        key,
        priorTxnId: existing.id,
      });
      return null;
    }
    return existing;
